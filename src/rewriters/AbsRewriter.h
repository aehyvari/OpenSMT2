...
class AbsConfig : public DefaultRewriterConfig {
    ...
    PTRef freshAbs(PTRef abs) {
        ...
        assert(logic.yieldsSortInt(abs) or logic.yieldsSortReal(abs));
        return logic.yieldsSortInt(abs) ? logic.mkIntVar(id.c_str()) : logic.mkRealVar(id.c_str());
    }
...
public:
    ...
    PTRef rewrite(PTRef term) override {
        ...
        if (logic.isAbs(symRef)) {
            ...
            if (not inCache) {
                definitions.push(logic.mkEq(term, logic.mkIte(logic.mkGeq(term, logic.getZeroForSort(logic.getSortRef(term))), term, logic.mkNeg(term))));
            }
            return rewritten;
        }
        return term;
...
public:
    ...
    PTRef rewrite(PTRef term) override {
        if (term == PTRef_Undef or logic.yieldsSortInt(term) or logic.yieldsSortReal(term)) {
