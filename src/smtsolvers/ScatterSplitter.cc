...
{
    if (firstTime or shouldLearnClause)
    {
        if (firstTime)
        {
            firstTime = false;
        ...
        shouldLearnClause = false;
        std::vector<std::pair<string ,int>>  toPublishTerms;
        if (learnSomeClauses(toPublishTerms))
        {
            std::scoped_lock<std::mutex> lk(channel.getMutex());
            channel.assign(toPublishTerms);
        }
    }
}
...
bool ScatterSplitter::learnSomeClauses(std::vector<std::pair<string ,int>> & toPublishTerms) {
    ...
    for (int i = this->trail_sent; i < trail_max; i++) {
        ...
        toPublishTerms.push_back(std::make_pair(this->theory_handler.getLogic().printString(pt), 0));
    }
    ...
    for (int i = 0; i < this->learnts.size(); i++)
    {
        ...
        toPublishTerms.push_back(std::make_pair(this->theory_handler.getLogic()
            .printString(theory_handler.getLogic().mkOr(clause)), level));

        c.mark(3);
    }
    if (not toPublishTerms.empty()) {
        return true;
    }
    else return false;
};